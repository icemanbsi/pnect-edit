...
class WBaseAdminAppParams extends UParamsWorklet {
	...
	public function properties() {
		return array(
			'elements' => array(
				...
				'poweredBy' => array('type' => 'UI18NField', 'attributes' => array(
						'type' => 'text',
						'languages' => wm()->get('base.language')->languages(),
					), 'layout' => "{label}\n<fieldset>{input}</fieldset>\n{hint}",
					'hint' => $this->t('Please note that you can remove "Powered by Pinnect" only if you have ordered "branding removal" from us.')),
				...
				'inviteOnly' => array('type' => 'radiolist', 'items' => array(
						1 => $this->t('Enable: only invited users can sign up.'),
						0 => $this->t('Disable: any user can sign up.')
					), 'layout' => "{label}\n<fieldset>{input}\n{hint}</fieldset>"),
	...
	{	
		...
		$config['params']['poweredBy'] = $this->model->poweredBy;
		UHelper::saveConfig(app()->basePath .DS. 'config' .DS. 'public' .DS. 'modules.php',$config);
	}
