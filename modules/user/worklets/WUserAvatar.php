...
class WUserAvatar extends UUploadWorklet
{
	...
	{
		return wm()->get('base.helper')->bin($this->resolveBinId());
	}
	...
	{	
		if($this->bin)
		{
			...
			$image->resize($this->param('fileOriginalResize'), $this->param('fileOriginalResize'), Image::AUTO);
			
			$thumbnail = new Image($this->bin->getFilePath('original'));			
			$thumbnail->resize($this->param('fileThumbnailResize'), $this->param('fileThumbnailResize'), Image::AUTO);

			$this->bin->put($image, 'original', 'UImageStorageFile');
			$this->bin->put($thumbnail, 'thumbnail', 'UImageStorageFile');
